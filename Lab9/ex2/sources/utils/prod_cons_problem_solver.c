...
void * producer_thread(void * dumb){
    while(true){
        ...
        if(produced_line == NULL){
            ...
            REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "waiting to all_source_read_sem.");            
            sem_wait(&(CTRL_STRUCT->all_source_read_sem));
            REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "got all_source_read_sem.");                        
            ...
            REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "unlock all_source_read_sem.");  
...
void _add_line_to_buffer(char * const line){
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "waiting to lock empty_count_sem.");                            
    sem_wait(&(CTRL_STRUCT->empty_count_sem));
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "got empty_count_sem.");                             
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "waiting to lock prod_sem.");                                
    sem_wait(&(CTRL_STRUCT->prod_sem));             
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "got prod_sem.");  
    ...
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "waiting to lock prod_idx_sem.");                            
    sem_wait(&(CTRL_STRUCT->semaphores[CTRL_STRUCT->prod_idx]));
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "got prod_idx_sem.");                                
    CTRL_STRUCT->buffer[CTRL_STRUCT->prod_idx] = line;
    sem_post(&(CTRL_STRUCT->semaphores[CTRL_STRUCT->prod_idx]));
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "unlocked prod_idx_sem.");                                
    sem_post(&(CTRL_STRUCT->prod_sem));
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "unlocked prod_sem.");  
    sem_post(&(CTRL_STRUCT->fill_count_sem));
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "sinalling fill_count_sem.");                               
}
...
void _sem_pos_to_given_value(sem_t * sem, int val, const char * who, const char * desc){
    ...
    sem_getvalue(sem, &curr_sem_val);
     while(curr_sem_val < CTRL_STRUCT->consuments_number){
        REPORT_IF_VERBOSE(who, pthread_self(), desc);
        sem_post(sem);
        sem_getvalue(sem, &curr_sem_val);
    }
}
...
char * _consume_buffer(){
    ...
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "waiting to lock fill_count_sem.");      
    sem_wait(&(CTRL_STRUCT->fill_count_sem));
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "got fill_count_sem.");
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "waiting to lock cons_sem.");      
    sem_wait(&(CTRL_STRUCT->cons_sem));      
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "got cons_sem.");
    ...
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "waiting to lock cons_idx_sem.");
    sem_wait(&(CTRL_STRUCT->semaphores[CTRL_STRUCT->cons_idx]));    
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "got cons_idx_sem.");
    ...
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "unlocked cons_idx_sem.");                                
    sem_post(&(CTRL_STRUCT->cons_sem));   
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "unlocked cons_sem.");
    sem_post(&(CTRL_STRUCT->empty_count_sem));   
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "signalled empty_count_sem.");    
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "returning consumption result.");                                                                 
    return line;
}

bool _is_something_to_consume(){
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "waiting to lock all_source_read_sem.");      
    sem_wait(&(CTRL_STRUCT->all_source_read_sem));
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "got all_source_read_sem.");
    ...
    if(CTRL_STRUCT->all_source_read == CTRL_STRUCT->producers_number && fill_count_sem_val == 0){
        sem_post(&(CTRL_STRUCT->all_source_read_sem));
        REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "returned all_source_read_sem.");
        sem_post(&(CTRL_STRUCT->fill_count_sem));      
        REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "incrementing fill_count_sem to help other consumer to terminate.");
        return false;
    }      
    sem_post(&(CTRL_STRUCT->all_source_read_sem));
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "returned all_source_read_sem.");  
