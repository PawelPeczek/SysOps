...
void * producer_thread(void * dumb){
    ...
    while(true){
        ...
        if(produced_line == NULL){
            REPORT_IF_VERBOSE("[PRODUCER] LINE PRODUCED", pthread_self(), "EOF");
            REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "waiting to lock elems_mutex.");            
            pthread_mutex_lock(&(CTRL_STRUCT->elems_mutex));
            REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "got elems_mutex.");                        
            ...
            REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "unlocked elems_mutex.");  
            if(should_inform == true){
                REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "Informing all consumer waiting at free_mutex.");                                                                      
                _inform_all_consumer_if_now_buff_not_empty();
            }
            REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "Producer terminating.");
            break;
        } else {
            REPORT_IF_VERBOSE("[PRODUCER] LINE PRODUCED", pthread_self(), produced_line);
            add_line_to_buffer(produced_line);
            REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "Informing consumers waiting on free_mutex");            
        }
    }        
    return (void *) NULL;
}
...
void add_line_to_buffer(char * const line){
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "waiting to lock prod_mutex.");                            
    pthread_mutex_lock(&(CTRL_STRUCT->prod_mutex));
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "got prod_mutex.");                                
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "waiting until buffer full.");                                
    _wait_until_buffer_full();
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "buffer now not full.");
    REPORT_BUFFER_INDEX_IF_VERBOSE("[PRODUCER]", pthread_self(), CTRL_STRUCT->prod_idx);                                 
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "waiting to lock prod_idx mutex.");                            
    pthread_mutex_lock(&(CTRL_STRUCT->mutexes[CTRL_STRUCT->prod_idx]));
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "got prod_idx mutex.");                                
    ...
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "unlocked prod_idx mutex.");                                
    pthread_mutex_unlock(&(CTRL_STRUCT->prod_mutex));
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "unlocked prod_mutex.");                                
}

void _wait_until_buffer_full(){
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "waiting to lock buffer_full_mutex.");                            
    pthread_mutex_lock(&(CTRL_STRUCT->buffer_full_mutex));
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "got buffer_full_mutex.");                            
    if(CTRL_STRUCT->elems_in_buff == CTRL_STRUCT->buff_size){
        REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "unlocked elems_mutex.");  
        REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "waiting for full condition change.");  
        pthread_cond_wait(&(CTRL_STRUCT->buffer_full), &(CTRL_STRUCT->buffer_full_mutex));
        REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "full condition change occured.");          
    }
    CTRL_STRUCT->prod_idx = (CTRL_STRUCT->prod_idx + 1) % CTRL_STRUCT->buff_size;
    pthread_mutex_unlock(&(CTRL_STRUCT->buffer_full_mutex));
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "unlocked buffer_full_mutex.");      
}

void _inform_one_consumer_if_now_buff_not_empty(){
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "trying to get buffer_empty_mutex.");
    pthread_mutex_lock(&(CTRL_STRUCT->buffer_empty_mutex));
    pthread_cond_signal(&(CTRL_STRUCT->buffer_empty));
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "sent \"now not empty signal\".");                
    pthread_mutex_unlock(&(CTRL_STRUCT->buffer_empty_mutex));
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "unlocked buffer_empty_mutex.");  
}
...
void _inform_all_consumer_if_now_buff_not_empty(){
    ...
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "sent \"now not empty broadcast\".");                
    pthread_mutex_unlock(&(CTRL_STRUCT->buffer_empty_mutex));
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "unlocked buffer_empty_mutex.");      
}
...
char * produce_line(){
    char * line_buffer;
    size_t size = 0;
    REPORT_IF_VERBOSE("[PRODUCER]", pthread_self(), "reading a line from source.");
...
void * consumer_thread(void * dumb){
    while(true){
        ...
        REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "Consumption done.");      
        if(line == NULL){
            REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "Terminating consumer.");      
            break;
...
char * _consume_buffer(){
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "waiting to lock cons_mutex.");      
    pthread_mutex_lock(&(CTRL_STRUCT->cons_mutex));
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "got cons_mutex.");      
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "waiting until buffer empty");          
    if(_wait_until_buffer_empty_and_decide_if_all_done() == true){
        pthread_mutex_unlock(&(CTRL_STRUCT->cons_mutex)); 
        return NULL;
    }
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "buffer now not empty.");
    REPORT_BUFFER_INDEX_IF_VERBOSE("[CONSUMER]", pthread_self(), CTRL_STRUCT->cons_idx);
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "waiting to lock cons_idx mutex.");
    pthread_mutex_lock(&(CTRL_STRUCT->mutexes[CTRL_STRUCT->cons_idx]));    
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "got cons_idx mutex.");
    char * line = CTRL_STRUCT->buffer[CTRL_STRUCT->cons_idx];
    CTRL_STRUCT->buffer[CTRL_STRUCT->cons_idx] = NULL;
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "waiting to lock elems_mutex.");                            
    pthread_mutex_lock(&(CTRL_STRUCT->elems_mutex));    
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "got elems_mutex.");                            
    CTRL_STRUCT->elems_in_buff --;
    pthread_mutex_unlock(&(CTRL_STRUCT->elems_mutex));            
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "unlocked elems_mutex.");                            
    pthread_mutex_unlock(&(CTRL_STRUCT->mutexes[CTRL_STRUCT->cons_idx]));
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "unlocked cons_idx mutex.");                                
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "Informing producer that is waiting on full_mutex.");         
    _inform_one_producer_if_now_buff_not_full();        
    pthread_mutex_unlock(&(CTRL_STRUCT->cons_mutex));   
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "unlocked cons_mutex.");   
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "returning consumption result.");                                                                 
    return line;
}

bool _wait_until_buffer_empty_and_decide_if_all_done(){
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "waiting to lock buffer_empty_mutex.");                                
    pthread_mutex_lock(&(CTRL_STRUCT->buffer_empty_mutex));
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "got buffer_empty_mutex.");                                
    if(CTRL_STRUCT->elems_in_buff == 0){
        if(CTRL_STRUCT->all_source_read == CTRL_STRUCT->producers_number){
            pthread_mutex_unlock(&(CTRL_STRUCT->buffer_empty_mutex));
            REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "unlocked buffer_empty_mutex.");                                         
            return true;
        }        
        REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "waiting to change buffer_empty condition.");                                
        pthread_cond_wait(&(CTRL_STRUCT->buffer_empty), &(CTRL_STRUCT->buffer_empty_mutex));
        REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "change of buffer_empty condition occured.");                                
    }
...
void _inform_one_producer_if_now_buff_not_full(){
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "waiting to lock buffer_full_mutex.");                                
    pthread_mutex_lock(&(CTRL_STRUCT->buffer_full_mutex));
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "got buffer_full_mutex.");                                
    pthread_cond_signal(&(CTRL_STRUCT->buffer_full));
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "sent \"now not full signal\".");                    
    pthread_mutex_unlock(&(CTRL_STRUCT->buffer_full_mutex));
    REPORT_IF_VERBOSE("[CONSUMER]", pthread_self(), "unlocked buffer_full_mutex.");                                
}
