...
void _handle_new_client(int conn_fd, msg_with_name * req, struct sockaddr * addr, socklen_t addrlen);
int _insert_client_to_serv_structs(const char * name, struct sockaddr * addr, socklen_t addrlen, int fd);
...
void _handle_incomming_message(int conn_fd){
    int msg_type;
    if(recvfrom(conn_fd, (void *)&msg_type, sizeof(int), MSG_PEEK | MSG_NOSIGNAL, NULL, NULL) == -1){
...
void _clear_dumb_data(int conn_fd, size_t size){
    ssize_t size_read;
    char buff[1024];
    if((size_read = recvfrom(conn_fd, (void *)buff, sizeof(buff), MSG_NOSIGNAL, NULL, NULL)) == -1){
        perror("Cannot read from socket");
        exit(20);
    }
    if(size != 0 && size != size_read){
...
void _pong_received(int conn_fd){
    ...
    socklen_t addrlen; 
    msg_with_name pong;
    ssize_t size_read = recvfrom(conn_fd, (void *)&pong, sizeof(msg_with_name), MSG_NOSIGNAL, &addr, &addrlen);
...
void _conn_req_received(int conn_fd){
    ...
    ssize_t size_read = 
        recvfrom(conn_fd, (void *)&conn_req, sizeof(msg_with_name), MSG_NOSIGNAL, &addr, &addrlen);
    if(size_read == sizeof(msg_with_name)){
        _handle_new_client(conn_fd, &conn_req, &addr, addrlen);
    } else {
...
void _handle_new_client(int conn_fd, msg_with_name * req, struct sockaddr * addr, socklen_t addrlen){
    ...
    if(get_client_by_name(CLIENTS_HM, req->name) != NULL){
        ...
        printf("name %s is free\n", req->name);
        if(_insert_client_to_serv_structs(req->name, addr, addrlen, conn_fd) == -1){
            response.conn_accepted = false;
        } else {
            response.conn_accepted = true;
        }
    }
    int op_status = sendto(conn_fd, (const void *)&response, sizeof(conn_resp), MSG_NOSIGNAL, addr, addrlen); 
    if(response.conn_accepted == true && op_status == -1){
        pthread_mutex_lock(&clients_mutex);
...
int _insert_client_to_serv_structs(const char * name, struct sockaddr * addr, socklen_t addrlen, int fd){
    ...
    struct sockaddr * cl_all_addr = (struct sockaddr *)calloc(1, (size_t)addrlen);
    if(client == NULL || cl_all_addr == NULL){
        if(client != NULL) {
            free(client);
        }
    ...
    memcpy(cl_all_addr, addr, (size_t)addrlen);
    pthread_mutex_lock(&clients_mutex);
...
void _try_send_op_request_having_mutex(op_request * request, client_entry * client){
    ...
    ssize_t sent_bytes = sendto(client->socket_fd, (const void *)request, sizeof(op_request), MSG_NOSIGNAL, client->dest_addr, client->addrlen);
...
void _op_resp_received(int conn_fd){
    ...
    ssize_t bytes_read = recvfrom(conn_fd, (void *)&response, sizeof(op_response), MSG_NOSIGNAL, NULL, NULL);
...
void _ping_all(){
    ...
    while(it != &(CLIENTS_LIST->right_sent)){
        ...
        ssize_t sent_bytes = sendto(it->client->socket_fd, (const void *)&ping, sizeof(msg_with_name), MSG_NOSIGNAL, it->client->dest_addr, it->client->addrlen);
...
void _deallocate_client_entries(){
    ...
    while(it != &(CLIENTS_LIST->right_sent)){
        if(it->client != NULL){
            ...
            list_node * to_del = it;
            it = it->next;
            sendto(to_del->client->socket_fd, (const void *)&kill, sizeof(message_only_type), MSG_NOSIGNAL, to_del->client->dest_addr, to_del->client->addrlen);
            free(to_del->client->dest_addr);
            free(to_del->client);
        }
    }
}
